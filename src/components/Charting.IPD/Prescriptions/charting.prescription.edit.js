...
import { LabTestSearchInput, DrugsSearchInput } from '../../SearchBar';
...
class PrescriptionEdit extends React.Component<Props, State> {
  componentWillMount() {
    ...
    data.forEach((item) => {
      formData.push({
        ...
        food_precedence: item.food_precedence,
  ...
  onDelete = () => {
    const { visitId } = this.props;
    this.props.deletePrescriptions(visitId);
  };
  onDeleteDrug = (index) => {
...
const Wrapped = Form.create({
  mapPropsToFields(props: Props) {
    ...
    data.forEach((item, index) => {
      ...
      fields.IDrugId[index] = Form.createFormField({ value: item.drug_id });
      fields.IDrugName[index] = Form.createFormField({ value: item.drug.name });
      fields.IIntake[index] = Form.createFormField({ value: item.intake });
      fields.IFrequency[index] = Form.createFormField({ value: item.frequency });
      fields.IFoodPrecedence[index] = Form.createFormField({ value: item.food_precedence });
      fields.IDuration[index] = Form.createFormField({ value: item.duration });
      fields.IDurationUnit[index] = Form.createFormField({ value: item.duration_unit });

      fields.IInstruction[index] = Form.createFormField({ value: item.instruction });
...
const mapDispatchToProps = dispatch => ({
  ...
  deletePrescriptions: visitId => dispatch(deletePrescriptions(visitId)),
});
